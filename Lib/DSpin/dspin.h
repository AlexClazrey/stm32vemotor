...
#define dSPIN_BUSY_Pin		GPIO_PIN_2
#define dSPIN_BUSY_Port		GPIOC

#define dSPIN_FLAG_Pin		GPIO_PIN_1
...
typedef enum {
	...
	dSPIN_ALARM_EN_ALL_ENABLE = ((uint8_t) 0xFF)
...
typedef enum {
	FWD = ((uint8_t) 0x01), REV = ((uint8_t) 0x00)
...
#define KTherm_to_Par(KTherm) ((uint8_t)(((KTherm - 1)/0.03125)+0.5))			/* 热补偿值, 范围： 1 to 1.46875 */
#define StallTh_to_Par(StallTh) ((uint8_t)(((StallTh - 31.25)/31.25)+0.5))		/* 失速阀值, 范围： 31.25mA to 4000mA */
